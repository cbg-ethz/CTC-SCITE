...
std::string runMCMCnew(vector<struct treeTheta>& bestTrees, double* errorRates, int noOfReps, int noOfLoops, double gamma, std::vector<double> moveProbs, int n, int m, int** dataMatrix, char scoreType, int* trueParentVec, int step, bool sample, double chi, double priorSd_beta, double priorSd_alpha, bool useTreeList, char treeType, bool doubletModel, double doubletRate, int recMut, int z, double alphaMoveRatio, bool fixedDoubletProb);
