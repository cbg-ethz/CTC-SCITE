...
compute_pairwise_distance_of_leaves <- function(treeData, leaf1, leaf2, nCells,
  ...
  ancestorMatrix <- parentVector2ancMatrix(
  ...
  bestMutationPlacement <- getMutationPlacement(
    ...
    wbcStatus
  ) ## This crashes when executed on posterior sampling of Br61
    ...
    error = function(e) {
      plot(
        ggplot(data, aes(x = log(StatisticsOfMutationPlacement))) +
          geom_histogram(bins = 10, fill = "skyblue", color = "skyblue", alpha = 0.7) +
          xlab("Maximal probability of branching evolution") +
          ylab("total count") +
    ...
    system.time(
      for (it in cellPairSelection) {
        ...
        posterior <- produce_Distance_Posterior(leaf1, leaf2, postSampling, treeName, nCells,
          ...
          nSamplingEvents = nMutationSamplingEvents
        )
        ...
        for (i in cellsInCluster) {
          ...
          while (j < i) {
            ...
            posterior <- produce_Distance_Posterior(i, j, postSampling, treeName, nCells,
              ...
              alleleCount, sampleDescription$Cluster,
              mutatedReadCounts, totalReadCounts, sampleDescription$WBC,
              nSamplingEvents = nMutationSamplingEvents, clusterName = it
            )
