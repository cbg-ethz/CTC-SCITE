...
Collect a bunch of pairs of cells that are genetically identical or similar:
...
```
There is no pair of cells that is just above the cutoff.

So I stick with the one pair that I found.

This computes baseline distance histograms for single cells in the tree.

```{r baseline distance distributions}

nTreeSamplingEvents <- 50
