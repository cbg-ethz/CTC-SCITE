...
data.frame(x = mutatedReadCounts %>% sapply(function(x){sum(x!=0)/length(x)})) %>%
  ...
  geom_line(data = curve_data, aes(x = x, y = y), color = "red", linewidth = 1) 
...
```
There is no pair of cells that is below the cutoff (1).


Collect a bunch of pairs of cells that have distance 4:
...
These might also be suitable for simulating 2-cell CTC cluster.
