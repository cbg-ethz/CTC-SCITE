...
//               separating mutations for leaf pairs, frequency of tree
//               splits,...
//============================================================================
...
vector<vector<int>> getExpVarAlleleCount(int m, int sampleCount,
                                         bool **ancMatrix,
                                         vector<int> &leafClusterId);
double logBinomCoeff(int n, int k);
double logBetaBinom(int k, int r, int alpha, int beta, int p, int q,
                    double tau);
double computeLogSampleScore(int expCount, int alleleCount, int obsMutCount,
                             int obsTotalCount, double dropoutRate,
                             double seqErr, double tau);
int sampleEvent(std::vector<double> prob);
vector<int> getMutPlacement(int m, int n, int sampleCount, bool **ancMatrix,
                            vector<int> &alleleCount,
                            vector<int> &leafClusterId,
                            vector<vector<int>> &mutReadCounts,
                            vector<vector<int>> &totalReadCounts,
                            double dropoutRate, double seqErr, double tau,
...
int getLCA(int v, int w, int *parentVector, vector<vector<bool>> &ancMatrix,
           int leafCount);
...
int getHPA(int v, vector<vector<bool>> &sameOrigCluster, int *parentVector,
           vector<vector<bool>> &ancMatrix, int leafCount);
void updateSplitCounts(std::map<string, int> &splitCounts,
                       vector<vector<bool>> &ancMatrix,
                       vector<string> &leafLabels, int nodeCount, int *parent);
...
int main(int argc, char *argv[]) {
  ...
  vector<vector<string>> sampleInfo; // the original table of the samples (ids,
                                     // cell count, type,...)
  int sampleCount;

  vector<vector<int>> mutReadCounts;   // entry[i][j] has mutated read count for
                                       // position i in sample j
  vector<vector<int>> totalReadCounts; // entry[i][j] has total read count for
                                       // position i in sample j

  vector<string> leafLabels;
  vector<int> leafClusterId;
  vector<int> alleleCount;
  vector<bool> wbcStatus; // wbcStatus[i] is 1 if leaf i represents a white
                          // blood cell, 0 otherwise
  ...
  mutReadCounts =
      getMutReadCount(readCounts); // count of mutated reads after filtering per
                                   // position per sample
  totalReadCounts = getTotalReadCount(
      readCounts); // total read count after filtering per position per sample
  cout << "sample count = " << mutReadCounts.at(0).size() << endl;

  // get leaf labels and sample sizes
  cout << descriptionFileName << endl;
  sampleInfo =
      readStringTableFile(descriptionFileName); // the original table of the
                                                // samples (ids and cell count)
  ...
  int parentVectorSize =
      (2 * m) -
      2; // binary tree: m leafs and m-1 inner nodes, root has no parent

  vector<vector<int>> mutPlacementCounts; // counts for each mutation and each
                                          // tree node how often the mutation
  for (int i = 0; i < n;
       i++) { // is placed at this node in the posterior sampling
    vector<int> vect((2 * m) - 1, 0);
    mutPlacementCounts.push_back(vect);
  }

  /******************************************************************************************************/
  /*  This is the new initialisation where we count for each mutation pair and
   * each original cluster    */
  /*  how often the pair co-occurs in one of the clusters's private branches. */
  /*  Here a private branch is defined as the path up from a leaf just before
   * the lca of the leaf       */
  /*	and another leaf of the same cluster. If the original cluster was broken
   * into multiple samples,   */
  /*	we can also define private branches for each unbroken (multi-cell)
   * sample. That means a branch is */
  /*	private until an lca with a cell from one of the other (multi-cell)
   * samples is reached.           */
  /******************************************************************************************************/

  vector<vector<vector<int>>>
      coOccCount; // for each leaf and each mutation pair count how often
  for (int leaf = 0; leaf < leafClusterId.size();
       leaf++) { //  the pair co-occurs in the private lineage of the leaf
  ...
  vector<vector<bool>> sameOrigCluster; // Is '1' if cell i and cell j are from
                                        // same original cluster (same color)
  for (int leaf = 0; leaf < leafClusterId.size();
       leaf++) { // Initialize default to '0'
  ...
  for (int cell_1 = 0; cell_1 < leafClusterId.size(); cell_1++) {
    ...
    if (color1 == "gray93" ||
        color1 == "ghostwhite") { // Skip sample if it has no proper color,
    ...
    for (int cell_2 = 0; cell_2 < leafClusterId.size(); cell_2++) {
      ...
      if (color2 == "gray93" ||
          color1 == "ghostwhite") { // Skip if second sample is separately
                                    // sequenced single cell
        continue;
      }
      if (getNodeColor(leafLabels.at(cell_1)) ==
          getNodeColor(leafLabels.at(cell_2))) { // both samples have same color
        sameOrigCluster.at(cell_1).at(cell_2) =
            true; // so they're from same orig cluster
      }
    }
  }

  /* counts for each leaf pair the average number of separating mutations
   * (minimum of both branches)*/
  ...
  /******************************************************************************************/
  /*    Now we read the file containing the posterior samples, each line is one
   * sample,     */
  ...
  while (getline(f, line)) {
    ...
    bool **currAncMatrix =
        parentVector2ancMatrix(currParentVector, parentVectorSize);
    vector<vector<bool>> currAncMatrixWithRoot =
        parentVector2ancMatrixWithRoot(currParentVector, parentVectorSize);
    vector<int> mutPlacement =
        getMutPlacement(m, n, sampleCount, currAncMatrix, alleleCount,
                        leafClusterId, mutReadCounts, totalReadCounts,
                        currDropOutRate, currSeqErrRate, currLogTau, wbcStatus);

    /* Update the Split Counts for Francesco */
    updateSplitCounts(splitCounts, currAncMatrixWithRoot, leafLabels,
                      (2 * m) - 2, currParentVector);

    for (int mut = 0; mut < n; mut++) {
      mutPlacementCounts.at(mut).at(mutPlacement.at(mut)) +=
          1; // update counts how often a mutation is placed at
    }        // at a tree node in the posterior sampling

    vector<int>
        leafHPA; // get for each leaf the highest ancestor in private branch
    for (int v = 0; v < leafLabels.size();
         v++) { //  (right below the first lca with other leaf from same
                //  original cluster)
      leafHPA.push_back(getHPA(v, sameOrigCluster, currParentVector,
                               currAncMatrixWithRoot, leafLabels.size()));
    }
    // cout << "HPA done" << endl;
    for (int i = 0; i < leafLabels.size();
         i++) { // go through all leafs and mutation pairs
      // cout << getActualLabel(leafLabels.at(i)) << endl;	  // update
      // co-occurrence counter if the pair
      for (int mut_1 = 0; mut_1 < n;
           mut_1++) { // co-occurs in the leafs private lineage
        int mutNode_1 = mutPlacement[mut_1];

        // skip if first mutation is not in private lineage of leaf
        if (!(currAncMatrixWithRoot[mutNode_1][i] &&
              currAncMatrixWithRoot[leafHPA.at(i)][mutNode_1])) {
          continue;
        }
        ...
        for (int mut_2 = 0; mut_2 < n; mut_2++) {
          ...
          if (!(currAncMatrixWithRoot[mutNode_2][i] &&
                currAncMatrixWithRoot[leafHPA.at(i)][mutNode_2])) {
            continue;
          }
    ...
    // count the number of separating mutations for all leaf pairs, i.e. the
    // number of mutation s
    for (int i = 0; i < leafLabels.size(); i++) {
      for (int j = i + 1; j < leafLabels.size(); j++) {
        int counter1 = 0;
        int counter2 = 0;
        int lca = getLCA(i, j, currParentVector, currAncMatrixWithRoot,
                         leafLabels.size());
        for (int mut = 0; mut < n; mut++) {
          int mutNode = mutPlacement[mut];
          if (currAncMatrixWithRoot[lca][mutNode] &&
              currAncMatrixWithRoot[mutNode][i] && mutNode != lca) {
            counter1++;
          }
          if (currAncMatrixWithRoot[lca][mutNode] &&
              currAncMatrixWithRoot[mutNode][j] && mutNode != lca) {
            counter2++;
          }
        }
        numberOfMinSeparatingMuts[i][j] += min(counter1, counter2);

        // For getting the histogram of min separation counts for ambiguous
        // cluster in Pr9
        //				if(getActualLabel(leafLabels.at(i))=="\"Pr9_CTC_26_A\""
        //&& getActualLabel(leafLabels.at(j))=="\"Pr9_CTC_26_B\""){
        //					sepMutCountHist.push_back(min(counter1,
        // counter2));
        //				}
      }
    }
    ...
    //	    					if(swap){
    //	    						separatingMuts_2.at(i).at(j).at(mut)
    //+= 1;
    //	    					}
    //	    					else{
    //	    						separatingMuts_1.at(i).at(j).at(mut)
    //+= 1;
    //	    					}
    //	    				}
    //	    			}
  ...
  /* output minimum number of separating mutations per cluster (average over all
   * pairs) */
  stringstream dummyOutFileName;
  dummyOutFileName << prefix << "_sepMinStats.txt";
  ofstream sepMinStatsOutput(
      dummyOutFileName.str()); // dummy output for average number of separating
                               // mutations (sum branches)
    ...
    for (int i = 0; i < clusterList.size(); i++) {
      for (int j = i + 1; j < clusterList.size(); j++) {
        ...
        sepMinStatsOutput << getActualLabel(leafLabels.at(clusterList.at(j)))
                          << "   ";
        sepMinStatsOutput << (1.0 * numberOfMinSeparatingMuts[clusterList.at(
                                        i)][clusterList.at(j)]) /
                                 counter
                          << endl;
      }
  ...
  for (int leaf = 0; leaf < leafClusterId.size(); leaf++) {
    for (int leaf2 = leaf + 1; leaf2 < leafClusterId.size(); leaf2++) {
      ...
      scatterSepMinStatsOutput
          << (1.0 * numberOfMinSeparatingMuts[leaf][leaf2]) / counter << endl;
    }
  }

  // For getting the histogram of min separation counts for ambiguous cluster in
  // Pr9
  ...
  for (int leaf = 0; leaf < leafClusterId.size(); leaf++) {
    ...
    dummyOutFileName2 << prefix << "_" << getNodeColor(leafLabels.at(leaf))
                      << ".txt";
    ofstream distOutput(
        dummyOutFileName2
            .str()); // outfile for table with pairwise mutation distances
    ...
    for (int mut_1 = 0; mut_1 < n; mut_1++) {
      ...
      for (int mut_2 = 0; mut_2 < n; mut_2++) {
        ...
        //					//tableCoOccOutput << "\t" <<
        // coOccCount.at(i).at(j).at(mut_1).at(mut_2);
      }
      distOutput << endl;
    }
    distOutput << endl;
  ...
  //	ofstream tableCoOccOutput (dummyOutFileName.str());             // dummy
  // output for posterior co-occurrence probability 	if
  //(!tableCoOccOutput.is_open()){ 		cout << "Unable to open file ";
  //	}
  ...
  //		for(int j=i+1; j<leafLabels.size(); j++){
  //			stringstream temp;
  //			temp << getActualLabel(leafLabels.at(i)) << "-" <<
  // getActualLabel(leafLabels.at(j)); 			string leafPair =
  // temp.str();
  // leafPair.erase(std::remove(leafPair.begin(), leafPair.end(),
  //'\"'), leafPair.end()); 			tableCoOccOutput << leafPair <<
  // endl; for(int
  // mut_2=0; mut_2<n; mut_2++){ 				tableCoOccOutput
  // <<
  // "\t"
  // << mutLabels.at(mut_2);
  //			}
  ...
  //					}
  //					else{
  //						tableCoOccOutput << "\t" << 1.0
  //- (1.0*coOccCount.at(i).at(j).at(mut_1).at(mut_2)/counter);
  //					}
  //					//tableCoOccOutput << "\t" <<
  // coOccCount.at(i).at(j).at(mut_1).at(mut_2);
  //				}
  ...
  //
  //	string tableFile = prefix + "_table.tsv";     // output file containing
  // for each mutation and each tree node how 	ofstream tableOutput
  // (tableFile);
  //// often the mutation was placed at that node in the posterior sample
  /// if
  //(!tableOutput.is_open()){ 		cout << "Unable to open file";
  //	}
  //	cout << tableFile << endl;
  //	//getchar();
  //	for(int mut=0; mut<n; mut++){
  ...
  //	string privFile = prefix + "_privMuts.tsv";  // output file containing
  // for each leaf pair the average number of 	ofstream privOutput (privFile);
  // // private mutation in each of the two leaf lineages and average 	if
  //(!privOutput.is_open()){                  // number of other mutations
  // cout
  //<< "Unable to open file";
  //	}
  //	cout << privFile << endl;
  //	privOutput << "leaf_1" << "\t" << "leaf_2" << "\t" << "privateMuts_1" <<
  //"\t" <<  "privateMuts_1" << "\t" << "otherMuts" << endl; 	for(int i=0;
  // i<leafLabels.size(); i++){ 		for(int j=i+1;
  // j<leafLabels.size(); j++){ 			privOutput <<
  // getActualLabel(leafLabels.at(i)) << "\t" <<
  // getActualLabel(leafLabels.at(j)) << "\t" <<
  //(1.0*privMuts_1.at(i).at(j))/counter << "\t"; privOutput
  //<< (1.0*privMuts_2.at(i).at(j))/counter << "\t"
  //<< 1.0*(otherMuts.at(i).at(j))/counter << "\t"; privOutput
  //<<
  //(1.0*(privMuts_1.at(i).at(j)+privMuts_2.at(i).at(j)+otherMuts.at(i).at(j)))/counter
  //<< "\t" << counter << endl;
  //		}
  //	}

  //	string sepFile = prefix + "_separationStat.tsv";       // output file
  // listing for each leaf pair and each mutation the 	ofstream sepOutput
  //(sepFile);                          // probability that the mutation is in
  // the left or right 	if (!sepOutput.is_open()){ // private lineage and the
  // max and min of these values 		cout << "Unable to open file";
  //	}
  //	cout << sepFile << endl;
  //	for(int i=0; i<leafLabels.size(); i++){
  //		for(int j=i+1; j<leafLabels.size(); j++){
  //			sepOutput << getActualLabel(leafLabels.at(i)) << "\t" <<
  // getActualLabel(leafLabels.at(j)) << endl; 			for(int mut=0;
  // mut<n;
  // mut++){ 				sepOutput << mutLabels.at(mut) << ":\t";
  // sepOutput
  //<< 1.0*separatingMuts_1.at(i).at(j).at(mut)/counter << "\t";
  // sepOutput
  //<< 1.0*separatingMuts_2.at(i).at(j).at(mut)/counter << "\t";
  // sepOutput <<
  // max(1.0*separatingMuts_1.at(i).at(j).at(mut)/counter, 1.0*separatingMuts_2.at(i).at(j).at(mut)/counter)
  //<< "\t"; 				sepOutput <<
  // min(1.0*separatingMuts_1.at(i).at(j).at(mut)/counter, 1.0*separatingMuts_2.at(i).at(j).at(mut)/counter)
  //<< endl;
  //			}
  //		}
  //	}
  ...
  //		for(int j=i+1; j<leafLabels.size(); j++){
  //			//cout << "\nTop separators for " <<
  // getActualLabel(leafLabels.at(i)) << " and " <<
  // getActualLabel(leafLabels.at(j)) << ":" << endl;
  // topSepOutput
  // <<
  // "\nTop separators for " << getActualLabel(leafLabels.at(i)) << " and " <<
  // getActualLabel(leafLabels.at(j)) << ":" << endl; vector<int> copy_1 =
  // separatingMuts_1.at(i).at(j); 			vector<int> copy_2 =
  // separatingMuts_2.at(i).at(j); 			vector<string>
  // topSepMuts_1; vector<string>
  // topSepMuts_2; 			for(int top=0; top<10; top++){
  // int maxSepIndex=0; 				int maxSep=0; for(int
  // mut=0; mut<n; mut++){
  //					//cout <<
  // abs(copy_1.at(mut)-copy_2.at(mut))
  //<<
  // endl; 					int sep = copy_1.at(mut);
  // if(sep>maxSep){ 						maxSepIndex =
  // mut; maxSep = sep;
  //						//cout << "top: " << mut <<
  // endl;
  //					}
  ...
  //			}
  //			topSepOutput << getActualLabel(leafLabels.at(i)) << ":"
  //<<
  // endl; 			for(int t=0; t<10; t++){
  // topSepOutput
  // << topSepMuts_1.at(t) << endl;
  //			}
  //			topSepOutput << getActualLabel(leafLabels.at(j)) << ":"
  //<<
  // endl; 			for(int t=0; t<10; t++){
  // topSepOutput
  // << topSepMuts_2.at(t) << endl;
  //			}
  //			//cout << endl;
  //		}
  //	}
  ...
  //	};
  //
  //	std::set < std::pair<string, int>, decltype( cmp )>
  // s(splitCounts.begin(), splitCounts.end(), cmp);
  //
  //	for (const auto &p : s)
  //	{
...
/*  Compute the highest private ancestor of a leaf, i.e. the earliest ancestor
 * of a sample that is not an ancestor  */
...
//	    			/* Get for each sample/attachment point pair the
// probability of placement */ 	    			vector<vector<double> >
// currTable = getPlacementScoreTable(m, n, sampleCount, currTreeAncMatrix,
// alleleCount, leafClusterId, mutReadCounts, totalReadCounts, dropoutRate,
// seqErr, tau, bestPlacementPoint, wbcStatus);
//	    			//cout << "table size: " << currTable.size() <<
// endl;
//
//	    			/* compute for all leaf pairs, the separation
// score*/ 	    			string line =
// getSeparationScores(currTable, currTreeAncMatrix, n, m, currTreeParentVec,
// leafPairs); sampleOutput << line;
//	    			//cout << line << endl;
//
//
//	    			string partition =
// getPartition(currTreeAncMatrix, m, label);
// treeSampleOutput << partition;
//}

// string getSeparationScores(vector<vector<double> > table, bool** ancMatrix,
// int n, int m, int* parentVector, vector<string> leafPairs){
//	vector<vector<double> > normScoreTable;
//	for(int mut=0; mut<table.size(); mut++){
//		vector<double> normScores;
//		double bestLogScore = table.at(mut).at(0);           // best
// placement score of mut in the precomputed table
//
//		for(int node=0; node<table.at(mut).size(); node++){ // find the
// best placement score for mut 			bestLogScore =
// max(bestLogScore, table.at(mut).at(node));  // among all possible placement
// points
//		}
//
//		double tempScore = 0.0;
//		for(int node=0; node<table.at(mut).size(); node++){ // sum of
// placement scores for mutation 			tempScore +=
// exp(table.at(mut).at(node)-bestLogScore);   // subtraction of best score to
// calculate with score differences (smaller values)
//		}
//		double logSumScore = log(tempScore)+bestLogScore; // transform
// back to log scores and change from score differences to actual scores
//
//		//cout << exp(logSumScore) << endl;
//
//
//		for(int node=0; node<table.at(mut).size(); node++){
//			double normScore = table.at(mut).at(node)-logSumScore;
//// set table with normalized scores
// normScores.push_back(normScore);
//			//cout << "norm score mut " << mut << ": " <<
// exp(normScore)
//<< endl;
//		}
//		//getchar();
...
//			cout << "mut " << mut << ": " <<
// exp(normScoreTable.at(mut).at(node)) << " ";
//		}
//		cout << "\t";
//	}
//
...
//			int lca = getLCA(v, w, ancMatrix, m);    // lca of the
// two leafs
//			//cout << "lca: " << lca << endl;
//			//getchar();
//			sepLine << leafPairs.at(count++);
//
//			double v_expPathMutCount = getExpPathMutCount(v, lca,
// parentVector, normScoreTable); 			double w_expPathMutCount
// =
// getExpPathMutCount(w, lca, parentVector, normScoreTable);
// sepLine << "\t" << max(v_expPathMutCount,w_expPathMutCount) << "\t" <<
// v_expPathMutCount << "\t"
//<< w_expPathMutCount; 			for(int mut=0; mut<table.size();
// mut++){
//
//				double v_sep = mutSeparationOdds(mut, v, lca,
// parentVector, normScoreTable); 				double w_sep =
// mutSeparationOdds(mut, w, lca, parentVector, normScoreTable);
// double maxSep = max(v_sep,w_sep); 				sepLine  <<
//"\t" <<  maxSep;
//			}
//			sepLine << "\n";
//		}
...
//}
//
// double mutSeparationOdds(int mut, int v, int lca, int* parentVector,
// vector<vector<double> >& normScoreTable){
//
...
// double getExpPathMutCount(int v, int lca, int* parentVector,
// vector<vector<double> >& normScoreTable){
//
...
//	int currLCA = root;
//	//cout << "initLCA = " << currLCA << "  for " << v << " and " << w <<
// endl; 	for(int node=0; node<2*m-2; node++){
// if(ancMatrix[node][v] && ancMatrix[node][w]){   // common ancestor
// if(currLCA==root || ancMatrix[currLCA][node]){             // now also lowest
// common ancestor 				currLCA = node;
    ...
    if (strcmp(argv[i], "-i") == 0) { // name of data count file
      if (i + 1 < argc) {
        ...
        cout << dataFileName << endl;
               ...
               0) { // name of file describing the samples (i.e.
                    // sample_nodeDescription.tsv)
      if (i + 1 < argc) {
        descriptionFileName = argv[++i];
      }
    } else if (strcmp(argv[i], "-samples") ==
               0) { // name of file containing the MCMC samples
      if (i + 1 < argc) {
...
vector<vector<int>> getMutReadCount(vector<vector<int>> &readCounts) {
  ...
  for (int i = 0; i < readCounts.size(); i++) { // get for each mutation
    ...
    for (int j = 1; j < readCounts.at(i).size();
         j += 2) { // the count of mutated reads per sample
...
vector<vector<int>> getTotalReadCount(vector<vector<int>> &readCounts) {
  ...
  for (int i = 0; i < readCounts.size(); i++) {
    ...
    for (int j = 0; j < readCounts.at(i).size(); j += 2) {
      ...
      //				cout << "tot = " << readCounts[i][j] <<
      //"    mut = " << readCounts[i][j+1] <<  "   line = " << i << endl;
      //			}
      //			if(readCounts[i][j] < readCounts[i][j+1]){
      //				cout << "tot = " << readCounts[i][j] <<
      //"    mut = " << readCounts[i][j+1] <<  "   line = " << i << endl;
...
vector<string> getLeafLabels(vector<vector<string>> &sampleInfo) {
  ...
  for (int i = 0; i < sampleInfo.size(); i++) {
    ...
    //		cout << i <<  ": element count: " << sampleInfo.at(i).size() <<
    // endl; 		for(int p=0; p<sampleInfo.at(i).size(); p++){
    // cout
    // << "sample info: " << sampleInfo.at(i).at(p) << "\t";
    //		}
    //		cout << endl;
    ...
    for (int j = 1; j <= count; j++) {
      ...
      if (count > 1) {
        ...
        name = label.substr(left,
                            right - left); // sample name from sample name file
...
/** Goes through all cells in all clusters and sets the cell state to be wbc or
 * tumour. **/
...
vector<bool> getWbcStatus(vector<vector<string>> &sampleInfo) {
  ...
  for (int i = 0; i < sampleInfo.size(); i++) {
    int cellCount = stoi(
        sampleInfo.at(i).at(1)); // position 1 has total cell count in cluster
    int wbcCount = stoi(
        sampleInfo.at(i).at(3)); // position 3 has total wbc count in cluster
...
/***     computes the  number of alleles (normal and mutated) of a cell
 * (cluster) ***/
vector<int> getAlleleCount(vector<vector<string>> &sampleInfo) {
  vector<int> alleleCount;
  for (int i = 0; i < sampleInfo.size(); i++) {
...
vector<vector<int>> getExpAlleleCountFreqs(int m, int sampleCount,
                                           bool **ancMatrix,
                                           vector<int> &leafClusterId) {

  vector<vector<int>> expVarReadCount =
      getExpVarAlleleCount(m, sampleCount, ancMatrix, leafClusterId);
  ...
  for (int sample = 0; sample < sampleCount; sample++) {
    ...
    for (int i = 0; i < expVarReadCount.at(sample).size(); i++) {
      maxExpVarReadCounter =
          max(maxExpVarReadCounter, expVarReadCount.at(sample).at(i));
    }
    for (int i = 0; i <= maxExpVarReadCounter; i++) {
      freqCounter.push_back(
          0); // set frequency of each expected allele frequency to zero
    }

    for (int node = 0; node < (2 * m) - 1;
         node++) { // increase frequency counter of expected frequency
      freqCounter.at(expVarReadCount.at(sample).at(node)) +=
          1; // found at this node by one
    }
    ...
    //			cout << j << " expected: " << freqCounter.at(j) << "
    // times\t|\t"; 			sum += freqCounter.at(j);
    //		}
    //		cout << endl;
    //		if(sum != (2*m)-1){
    //			cout << "Error" << endl;
...
/** this computes for each node in the current tree how many wbc's are below it
 * (including node itself)**/
vector<int> getWBC_count_below(int m, int sampleCount, bool **ancMatrix,
                               vector<bool> &wbcStatus) {
...
/* precompute for each pair (sample, mutation placement) the number of expected
 * mutated alleles in the sample */
vector<vector<int>> getExpVarAlleleCount(int m, int sampleCount,
                                         bool **ancMatrix,
                                         vector<int> &leafClusterId) {
  ...
  for (int leaf = 0; leaf < m; leaf++) {
    ...
    for (int node = 0; node < (2 * m) - 2; node++) {
      if (ancMatrix[node][leaf] == 1) {
        expVarReadCount[clusterId][node] +=
            1; // count how many leafs belonging to the sample have
...
double logBinomCoeff(int n, int k) {
  ...
  //	cout << "total binom coeff: " <<
  // lgamma(n+1.0)-(lgamma(k+1.0)+lgamma(n+1.0-k)) << endl; 	if(k>n){
  // getchar();
  //		}
...
double logBetaBinom(int k, int r, int alpha, int beta, int p, int q,
  ...
  //	cout << "mu = " << mu << endl;
  //	cout << r << " chose " << k << endl;
  //	cout << "add:    " << lgamma(M) << " + " << logBinomCoeff(r,k) << " + "
  //<< lgamma(k+(M*mu)) << " + " << lgamma((r-k)+M*(1-mu)) << endl; 	cout <<
  //"substr: " << lgamma(M*mu) << "+" << lgamma(M*(1-mu)) << "+" << lgamma(r+M)
  //<< endl;

  logScore += lgamma(M) - (lgamma(M * mu) + lgamma(M * (1 - mu)));
  logScore += lgamma(r + 1.0) - (lgamma(k + 1.0) + lgamma(r + 1.0 - k));
  logScore +=
      lgamma(k + M * mu) + lgamma((r - k) + M * (1 - mu)) - lgamma(r + M);
...
double computeLogSampleScore(int expCount, int alleleCount, int obsMutCount,
  ...
  for (int p = 0; p < alpha; p++) {
    ...
    for (int q = 0; q < beta; q++) {
      ...
      logScore = logBinomCoeff(alpha, p) + logBinomCoeff(beta, q) +
                 (p + q) * log(delta) +
                 ((alpha - p) + (beta - q)) * log(1 - delta) +
                 logBetaBinom(k, r, alpha, beta, p, q, tau);
  ...
  for (int p = 0; p < alpha; p++) {
    double logScore = logBinomCoeff(alpha, p) + (beta + p) * log(delta) +
                      (alpha - p) * log(1 - delta) + logBinomCoeff(r, k) +
                      (r - k) * log(seqErr) + k * log(1 - seqErr);
    logScores.push_back(logScore);
  ...
  for (int q = 0; q < beta; q++) {
    double logScore = logBinomCoeff(beta, q) + (alpha + q) * log(delta) +
                      (beta - q) * log(1 - delta) + logBinomCoeff(r, k) +
                      k * log(seqErr) + (r - k) * log(1 - seqErr);
    logScores.push_back(logScore);

    //		cout << "logBinomCoeff(beta, q) = " << logBinomCoeff(beta, q) <<
    // endl; 		cout << "(alpha+q)*log(delta) = " <<
    // (alpha+q)*log(delta)
    // << endl; 		cout << "(beta-q)*log(1-delta) = " <<
    // (beta-q)*log(1-delta) << endl; 		cout << "logBinomCoeff(r, k) = "
    // << logBinomCoeff(r, k) << endl; 		cout << "k*log(seqErr) = " <<
    // k*log(seqErr) << endl; 		cout <<
    //"(r-k)* log(1-seqErr) = " << (r-k)* log(1-seqErr) << endl;
    //
    //
  ...
  double sumScore = 0.0;
  for (int i = 0; i < logScores.size();
       i++) { // sum over all allele states, exp is necessary as scores are
              // actually log scores
    sumScore += exp(logScores.at(i) -
                    maxLogScore); // subtraction of best score to calculate with
                                  // score differences (smaller values)
  }
  logSumScore =
      log(sumScore) + maxLogScore; // transform back to log scores and change
                                   // from score differences to actual scores
  // getchar();
  if (k > r) {
    cout << "k > r: " << k << " > " << r << endl;
    getchar();
...
int sampleEvent(
    std::vector<double> prob) { // picks an event based on the given the event
                                // probabilities, given probs need to sum to 1
...
vector<int> getMutPlacement(int m, int n, int sampleCount, bool **ancMatrix,
  ...
  vector<vector<int>> expVarAlleleCount =
      getExpVarAlleleCount(m, sampleCount, ancMatrix, leafClusterId);
  vector<vector<int>> expVarAlleleFreqs =
      getExpAlleleCountFreqs(m, sampleCount, ancMatrix, leafClusterId);
  vector<int> wbcBelowCount =
      getWBC_count_below(m, sampleCount, ancMatrix, wbcStatus);
  ...
  for (int mut = 0; mut < n;
    ...
    for (int sample = 0; sample < sampleCount; sample++) {
      ...
      for (int expFreq = 0; expFreq < expVarAlleleFreqs.at(sample).size();
           expFreq++) {
        // cout << "exp freq (" << mut << "): " << expFreq << "   allele count
        // at sample " << alleleCount.at(sample) << " (" <<
        // mutReadCounts.at(mut).at(sample) << "," <<
        // totalReadCounts.at(mut).at(sample) << ")" << endl;
        double newScore = computeLogSampleScore(
            expFreq, alleleCount.at(sample), mutReadCounts[mut][sample],
            totalReadCounts[mut][sample], dropoutRate, seqErr, exp(tau));
    ...
    for (int att = 0; att < (2 * m) - 1; att++) { // for each attachment point
      ...
      for (int sample = 0; sample < sampleCount; sample++) { // for each sample
        ...
        //				cout << mutReadCounts.at(0).size() <<
        // endl; 				cout <<
        // mutReadCounts[mut][sample]
        // << endl;

        double newScore = scorePrecomp.at(sample).at(expCount);
        //				double newScoreOld =
        // computeLogSampleScore(expCount, alleleCount.at(sample),
        // mutReadCounts[mut][sample], totalReadCounts[mut][sample],
        // dropoutRate, seqErr, tau);
        // if(newScore!=newScoreOld){ 					cout <<
        // "different scores" << endl; getchar();
        //				}
    ...
    for (int node = 0; node <= parentVectorSize; node++) {
      // cout << exp(logAttachmentScores.at(node)) << " / " << totalScore <<
      // endl;
...
bool **parentVector2ancMatrix(int *parent, int n) {
  ...
  for (int i = 0; i < n; i++) {
    int anc = i;
    int its = 0;
    while (anc < root) { // if the ancestor is the root node, it is not
                         // represented in the adjacency matrix
      if (parent[anc] < n) {
...
vector<vector<bool>> parentVector2ancMatrixWithRoot(int *parent, int n) {
  ...
  vector<vector<bool>>
      ancMatrix; // ancestor matrix with root represented  (0,1,...,n-1, n)
...
void updateSplitCounts(std::map<string, int> &splitCounts,
  ...
  for (int i = 0; i < nodeCount; i++) {
    ...
    for (int j = 0; j < leafLabels.size(); j++) {
      if (ancMatrix.at(i).at(j)) {
        ...
        if (isalpha(last) &&
            secondLast ==
                '_') { // remove last two characters if ends on "_A", "_B", etc.
          label.pop_back();
          label.pop_back();
        }
  ...
  for (int i = m; i < nodeCount; i++) {
    ...
    string temp1 = subTreeLabels.at(childNodes.at(0)) + " | " +
                   subTreeLabels.at(childNodes.at(1)) + " | all other samples";
    string temp2 = subTreeLabels.at(childNodes.at(1)) + " | " +
                   subTreeLabels.at(childNodes.at(0)) + " | all other samples";

    if (splitCounts.find(temp1) !=
        splitCounts.end()) { // if split is already in list increment count
      splitCounts[temp1] += 1;
    } else if (splitCounts.find(temp2) !=
               splitCounts.end()) { // if split is already in list (with swapped
                                    // branches) increment count
      splitCounts[temp2] += 1;
    } else {
      splitCounts.insert(std::pair<string, int>(
          temp1, 1)); // else insert into list and increment
    }
  }
}

// This is the version where we look at the subtree leaf labels
//		if(splitCounts.find(subTreeLabel) != splitCounts.end()){ // if
// split is already in list increment count
// splitCounts[subTreeLabel] += 1;
//		}
//		else{
//			splitCounts.insert (
// std::pair<string,int>(subTreeLabel,1)
//);  // else insert into list and increment
//		}
