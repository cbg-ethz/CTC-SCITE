...
d <- read.table(
  file.path(
    inputFolder, sample_name,
    paste0(sample_name, "_postSampling_", clusterName, ".txt")
  ),
  header = TRUE,
...
coverage <- read.table(
  file.path(
    inputFolder, sample_name, paste(sample_name, "covScore.txt", sep = "_")
  ),
...
selected_muts <- which(min_dist < 0.85) # select those below 0.5 say
...
diag(mat2) <- 1
min_dist <- apply(mat2, 1, min) # find minimum distance to other mutations
selected_muts <- which(min_dist < 0.56) # select those below 0.5 say
mat3 <- mat[selected_muts, selected_muts]
```
...
diag(mat2) <- 1
min_dist <- apply(mat2, 1, min) # find minimum distance to other mutations
selected_muts <- which(min_dist < 0.57) # select those below 0.5 say
mat3 <- mat[selected_muts, selected_muts]
```
...
abline(h = 0.66, lwd = 2, lty = 2, col = "green")
```
...
print(
  sprintf(
    ...
    clusterName, sum(top_muts_1 == "MODERATE")
  )
)
print(
  sprintf(
    "Number of mutations in cluster %s with high functional impact: %d",
    clusterName, sum(top_muts_1 == "HIGH")
  )
)
```
...
print(
  sprintf(
    "Number of mutations in cluster %s with moderate functional impact: %d",
    clusterName, sum(top_muts_2 == "MODERATE")
  )
)
print(
  sprintf(
    "Number of mutations in cluster %s with high functional impact: %d",
    clusterName, sum(top_muts_2 == "HIGH")
  )
)
```

Two genetic clusters could be identified in the 3-cell CTC cluster, but only
with moderate functional impact variants.
...
coverage <- read.table(
  ...
  header = TRUE, sep = "\t", stringsAsFactors = F, row.names = 1
)
coverage$variantName <- rownames(coverage)
head(coverage)
...
abline(h = 0.58, lwd = 2, lty = 2, col = "green")
```
...
# gold 
This is a 5-cell CTC cluster, so I am expecting up to 5 genetic clusters.
```{r}
clusterName <- "gold"
...
abline(h = 0.68, lwd = 2, lty = 2, col = "green")
```
...
We found two distinct genetic clusters. 
...
###Overview
Plotting the full distance matrix exceeds the C stack,so I do some soft
filtering of 
mutations:
...
diag(mat2) <- 1
min_dist <- apply(mat2, 1, min) # find minimum distance to other mutations
selected_muts <- which(min_dist < 0.93) # select those below 0.5 say
mat3 <- mat[selected_muts, selected_muts]
```
...
abline(h = 0.94, lwd = 2, lty = 2, col = "green")
```

```{r}
# geneGroups <- cutree(hc, k = NULL, h = 0.6)
geneGroups <- cutree(hc, k = 4)
```
...
```{r}
cluster1 <- names(geneGroups)[geneGroups == 4]
```
