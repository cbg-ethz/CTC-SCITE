library(tidyverse)
source('~/work/CTC-SCITE/experiments/assessing_cluster_clonality/workflow/scripts/simulateCTCclusters.R')
...
tree_names <- splitting_summary %>% dplyr::select(`Sample Name`) %>% unique()
...
for(idx in seq_len(nrow(splitting_summary))) {
  ...
  if(!tree_name == old_tree_name){
    ...
    variants_annotation <- read_delim(paste0('~/work/ctc-data/WES_experiment/splitting_summaries/', tree_name,'/', tree_name, '_variants_annotations.csv'))
  }
  ...
  splitting_summary$medium_impact_mutations[idx] <-
    called_variants_annotated %>% dplyr::filter(relevant == "MODERATE") %>% nrow()
