...
std::vector<double>
normalizeDistribution(const std::vector<double> &probabilityWeights) {
  double sum = 0;
  ...
  for (int i = 0; i < lengthOfVector; i++) {
    normalizedProbabilityWeights.push_back(
        exp(log(probabilityWeights[i]) - log(sum)));
  }
  return normalizedProbabilityWeights;
}

/* computes the probability mass function of the beta-binomial distribution */
double logBetaBinom(const int &k, const int &r, const int &alpha,
                    const int &beta, const int &p, const int &q,
                    const double &tau) {
...
std::vector<int>
drawWithReplacement(const std::vector<double> &probabilityWeights,
                    const int &sampleSize) {
  std::vector<double> probabilityDistribution =
      normalizeDistribution(probabilityWeights);
  std::discrete_distribution<int> distribution(probabilityDistribution.begin(),
...
transposeMatrix(const std::vector<std::vector<int>> &matrix, const int &nrows,
                const int &ncols) {
  std::vector<std::vector<int>> transposedMatrix(ncols,
...
std::vector<std::vector<int>>
getExpVarAlleleCount(const int &m, const int &sampleCount,
    ...
    for (int node = 0; node < (2 * m) - 2; node++) {
      if (ancMatrix[node][leaf] == 1) {
        expVarReadCount[clusterId][node] +=
            1; // count how many leaves belonging to the sample have
...
std::vector<std::vector<int>>
getExpAlleleCountFreqs(const int &m, const int &sampleCount,
                       const std::vector<std::vector<bool>> &ancMatrix,
                       const std::vector<int> &leafClusterId) {
  ...
  for (int sample = 0; sample < sampleCount; sample++) {
    ...
    for (int i = 0; i < expVarReadCount.at(sample).size(); i++) {
      maxExpVarReadCounter =
          std::max(maxExpVarReadCounter, expVarReadCount.at(sample).at(i));
    }
...
std::vector<int>
getWBC_count_below(int m, const std::vector<std::vector<bool>> &ancMatrix,
 ...
 *
 * @param expCount The estimated number of mutated alleles in the cell cluster
 *(alpha)
 * @param alleleCount The number alleles in the cell cluster. Equals number of
 * cells * 2 (alpha + beta)
 * @param obsMutCount The observed mutated allele count (after multiple
 *displacement amplification)
 * @param obsTotalCount The observed total allele count (after MDA)
 * @param dropoutRate (the inferred dropout rate)
 * @param seqErr (the inferred sequencing error rate (which is modelled as the
 *false positive rate))
 * @param tau
 *
 * @output The logarithm of the probability of k mutated reads under the
 *parameters given as input and the read count model
 *
 */
double computeLogSampleScore(int alpha, int alleleCount, int obsMutCount,
 ...
 *
 * @output An integer-valued vector which for each mutations indicates the node
 *it is optimally placed on.
 *
 */
// [[Rcpp::export]]
std::vector<int> getMutationPlacement(
    int m, int n, int sampleCount,
    const std::vector<std::vector<bool>> &ancMatrix,
    const std::vector<int> &alleleCount, const std::vector<int> &leafClusterId,
    const std::vector<std::vector<int>> &mutReadCounts,
    const std::vector<std::vector<int>> &totalReadCounts, double dropoutRate,
    double seqErr, double tau, const std::vector<bool> &wbcStatus) {
  ...
  std::vector<std::vector<int>> expVarAlleleCount =
      getExpVarAlleleCount(m, sampleCount, ancMatrix, leafClusterId);

  std::vector<std::vector<int>> expVarAlleleFreqs =
  ...
  for (int mut = 0; mut < n;
    ...
    /*  Given a fixed mutation, for all possible alphas and samples (determines
     alpha + beta) compute the probability of the observes read count */
    ...
    for (int sample = 0; sample < sampleCount; sample++) {
      ...
      for (int expFreq = 0; expFreq < expVarAlleleFreqs.at(sample).size();
           expFreq++) {
        // std::cout << "exp freq (" << mut << "): " << expFreq << "   allele
        // count at sample " << alleleCount.at(sample) << " (" <<
        // mutReadCounts.at(mut).at(sample) << "," <<
        // totalReadCounts.at(mut).at(sample) << ")" << std::endl;
        double newScore = computeLogSampleScore(
            expFreq, alleleCount.at(sample), mutReadCounts[mut][sample],
            totalReadCounts[mut][sample], dropoutRate, seqErr, tau);
    ...
    /* Given a mutation, for each attachment point compute the joint probability
       of observing the read counts and having a particular attachment, given
       tree and other model parameters. Report the optimal attachment point of
       the mutation.*/
    // cout << "end 2" << endl;
    for (int att = 0; att < (2 * m) - 1; att++) {
      ...
      //			cout << "mut placement list size: " <<
      // bestPlacementPoint.size() <<  " at mut " << mut << endl;
      // cout << "previous best: " << bestLogAttachmentScore << endl;
      // cout << " new: " << logAttachmentScore << endl;
      if (bestLogAttachmentScore <
          (logAttachmentScore -
           wbc_penalty)) { // update best placement point of mut in tree
      ...
      bestLogAttachmentScore =
          std::max(bestLogAttachmentScore, logAttachmentScore);
    }

    // double sumScore = 0.0;
    // for(int i=0; i<logAttachmentScores.size(); i++){              // sum over
    // all allele states, exp is necessary as scores are actually log scores
    //	sumScore += exp(logAttachmentScores.at(i)-bestLogAttachmentScore);   //
    // subtraction of best score to calculate with score differences (smaller
    // values)
    // }

    // double logMutScore = log(sumScore)+bestLogAttachmentScore; // transform
 ...
 *  otherwise it is a tumour cell
 *
 * @output An integer-valued matrix which for each mutations (row) gives an
 *unnormalized probability weight to be placed on a certain node (column).
 *
 */
// [[Rcpp::export]]
std::vector<std::vector<double>> computeMutationDistribution(
    int m, int n, int sampleCount, std::vector<std::vector<bool>> ancMatrix,
    const std::vector<int> &alleleCount, const std::vector<int> &leafClusterId,
    const std::vector<std::vector<int>> &mutReadCounts,
    const std::vector<std::vector<int>> &totalReadCounts, double dropoutRate,
    double seqErr, double tau, const std::vector<bool> &wbcStatus) {
  ...
  // std::vector<int> bestPlacementPoint;
  // for(int mut=0; mut< n; mut++){               // compute score separately
  // for each mutation
  //   bestPlacementPoint.push_back(-1);
  // }
  ...
  for (int mut = 0; mut < n;
    ...
    /* Given a mutation,compute the joint probability of observing the read
     counts and having a particular attachment, given tree and other model
     parameters. Report the optimal attachment point of the mutation.*/
    // cout << "end 2" << endl;
    for (int att = 0; att < (2 * m) - 1; att++) {
      // cout << "wbcBelowCount.at(" << att << ") = " << wbcBelowCount.at(att)
      // << endl;
      ...
      // cout << " new: " << logAttachmentScore << endl;
      // if(bestLogAttachmentScore <
      // logAttachmentScore){      // update best placement point of mut in tree
      // cout << "better placement" << endl;
      //    bestPlacementPoint.at(mut) = att;
      //}
      ...
      //  bestLogAttachmentScore = std::max(bestLogAttachmentScore,
      //  logAttachmentScore);
    }
 ...
 */
// [[Rcpp::export]]
std::vector<std::vector<int>> sampleMutationsPlacement(
    int nSamplingEvents, int nMutations,
  ...
  for (int m = 0; m < nMutations; m++) {
    ...
    for (int it = 0; it < logMutationPlacementProbabilities[m].size(); it++) {
      mutationPlacementProbabilities.push_back(
          exp(logMutationPlacementProbabilities[m][it]));
    }
    mutationPlacements.push_back(
        drawWithReplacement(mutationPlacementProbabilities, nSamplingEvents));
  }

  std::vector<std::vector<int>> mutationPlacementsTransposed =
      transposeMatrix(mutationPlacements, nMutations, nSamplingEvents);
...
parentVector2ancMatrix(const std::vector<int> &parent, int n) {
...
//      for (int j =0; j< i; j++) {
//        distance.push_back(produce_Distance_Posterior(cellsInCluster[i],
//        cellsInCluster[j], treePosterior, treeName));
...
// Input: a tree in parent vector format, meaning that the i'th entry of the
// vector
//  is te parent node of the entry i. Nodes are counted from zero and the root
//  is length(Tree)
// Output: A list with three entries:
//   - the first entry is a vector of nodes tracing back leaf 1 to the root
//   - the second entry is a vector of nodes tracing back leaf2 to the
//   descendant
 ...
 * @param leaf1 A number from 0 to the number of leaves-1 denoting the leaf
 * index
 ...
 *  - The fourth one is the shortest path between leaf1 and leaf 2. If
 * leaf1==leaf2 this will output leaf1.
 *
 */
// [[Rcpp::export]]
std::vector<std::vector<int>>
findMostRecentCommonAncestor(const std::vector<int> &treeParentVectorFormat,
                             int leaf1, int leaf2) {
  ...
  while (true) {
    ...
    if (lineage1.back() == treeParentVectorFormat.size())
      break;
  }
  ...
  while (std::find(lineage1.begin(), lineage1.end(), nextParent) ==
         lineage1.end()) {
  ...
  pathBetweenLeaves.insert(pathBetweenLeaves.end(), reversedLineage2.begin(),
                           reversedLineage2.end());
 ...
 * @param nCells The total number of Cells
 *
 * @output A matrix giving the probability of mutation i being placed on branch
 * j.
 *
 */
...
//    for(int j = 0;j< number of nodes in the tree(which is a function of
//    nCells); j++){
//      for(int l = 0; l<nCells, ++l){
//        double logSampleScore = computeLogSampleScore(expCount, alleleCount,
//        obsMutCount, obsTotalCount, dropoutRate, seqErr, 1)); double
//        attachmentPrior = ;
//
//      }
 ...
 * index
 * @param leaf2 like leaf1
 * @param mutationPlacement a vector of tree node indices indicating the
 * placement of the i'th mutation in its i'th entry
 * @param pairwiseGenealogy: The output of findMostRecentCommonAncestor
 *
 * @output the total number of mutations separating the genotypes of the two
 * leaves.
 *
 */
// [[Rcpp::export]]
std::vector<int> computePairwiseDistanceOfLeaves2(
    const std::vector<int> &treeParentVectorFormat, int leaf1, int leaf2,
    const std::vector<int> &mutationPlacement,
    const std::vector<std::vector<int>> &pairwiseGenealogy) {
  ...
  // std::vector<int> bestMutationPlacement = getMutationPlacement(nCells,
  ...
  for (int i = 0; i < mutationPlacement.size(); ++i) {
    if ((std::find(pathBetweenLeaves.begin(), pathBetweenLeaves.end(),
                   mutationPlacement[i]) != pathBetweenLeaves.end()) &&
        (mutationPlacement[i] != MRCA)) {
      // pathBetweenLeaves.end() && mutationPlacement[i] !=
      // firstLeafToMRCA[positionOfMRCA]) {
      distance++;
      if ((std::find(firstLeafToMRCA.begin(), firstLeafToMRCA.end(),
                     mutationPlacement[i]) != firstLeafToMRCA.end())) {
 ...
 * It computes for a pair of leaves (given by their pairwise genealogy) and
 * their two branches connecting them the maximal probability of any mutation to
 * land on either of the branches. It output the minimum of the two branaches.
 * Intuitiely, if there is no branching evolution, then at least one of the
 * branches should be populated with mutations with very low probability.
 *
 *
 * @param pairwiseGenealogy as output by findMostRecentCommonAncestor
 *
 * @param logMutationPlacementProbabilities As output by
 * computeMutationDistribution
 * @param nMutations The total number of mutations
 * @param nCells The total number of cells.
 *
 * @output The minimum (over the two branches) of the maximum (over all
 * mutations) of the probability of a mutation to land on one of the branches
 *
 */
// [[Rcpp::export]]
double ComputePerMutationProbabilityOfPolyclonality(
    const std::vector<std::vector<int>> &pairwiseGenealogy,
    const std::vector<std::vector<double>> &logMutationPlacementProbabilities,
  ...
  auto it =
      std::find(lineage1.begin(), lineage1.end(), pairwiseGenealogy[2][0]);
  ...
  for (int mut = 0; mut < nMutations; mut++) {
    ...
    mutationPlacementProbabilityWeights.reserve(
        logMutationPlacementProbabilities[mut].size());
    for (int it = 0; it < logMutationPlacementProbabilities[mut].size(); it++) {
      mutationPlacementProbabilityWeights.push_back(
          exp(logMutationPlacementProbabilities[mut][it]));
    }
    mutationPlacementProbabilities =
        normalizeDistribution(mutationPlacementProbabilityWeights);
    ...
    maxProbabilityOfMutationInLineage1 = std::max(
        maxProbabilityOfMutationInLineage1, ProbabilityOfMutationInLineage1);
    maxProbabilityOfMutationInLineage2 = std::max(
        maxProbabilityOfMutationInLineage2, ProbabilityOfMutationInLineage2);
  }

  // return mutationPlacementProbabilities;
  return std::min(maxProbabilityOfMutationInLineage1,
                  maxProbabilityOfMutationInLineage2);
}
 ...
 * counting the number of mutations that are on the shortest path from one leaf
 * to the other.
 *
 *
 * @param treeData The DataFrame containing the information from the posterior
 * Sampling
 * @param leaf1 A number from 0 to the number of leaves-1 denoting the leaf
 * index
 * @param leaf2 like leaf1
 ...
 * with entries denoting an index for the ClusterID. Corresponds to a number
 * from 0 to number of Clusters -1.
 * @param mutatedReadCounts A matrix with mutations as rows and clusters as
 * columns
 * @param totalReadCounts As mutatedReadCounts
 * @param wbcStatus A binary vector with as many entries as there are cells. 1
 * stands for being a white blood cell.
 *
 ...
 */
// [[Rcpp::export]]
std::vector<std::vector<double>> computePairwiseDistanceOfLeavesGivenTree(
    Rcpp::DataFrame treeData, int leaf1, int leaf2, int nCells, int nMutations,
    int nClusters, const std::vector<int> &alleleCount,
  ...
  // Preprocess tree
  std::vector<std::vector<bool>> ancestorMatrix = parentVector2ancMatrix(
      treeParentVectorFormat, treeParentVectorFormat.size());

  std::vector<std::vector<double>> logMutationPlacementProbabilities =
      computeMutationDistribution(nCells, nMutations, nClusters, ancestorMatrix,
                                  alleleCount, ClusterID, mutatedReadCounts,
                                  totalReadCounts, dropoutRate, seqErrRate, 1,
                                  wbcStatus);

  std::vector<std::vector<int>> mutationPlacements = sampleMutationsPlacement(
      nSamplingEvents, nMutations, logMutationPlacementProbabilities);

  // mutationPlacements.push_back(getMutationPlacement(nCells, nMutations,
  // nClusters,
  //                                                   ancestorMatrix,
  //                                                   alleleCount, ClusterID,
  //                                                   mutatedReadCounts,
  //                                                   totalReadCounts,
  //                                                   dropoutRate, seqErrRate,
  //                                                   1, wbcStatus));

  // Finding most recent common ancestor
  std::vector<std::vector<int>> pairwiseGenealogy =
      findMostRecentCommonAncestor(treeParentVectorFormat, leaf1, leaf2);
  ...
  for (int it = 0; it < mutationPlacements.size(); it++) {
    std::vector<int> output = computePairwiseDistanceOfLeaves2(
        treeParentVectorFormat, leaf1, leaf2, mutationPlacements[it],
        pairwiseGenealogy);
  ...
  std::vector<double> PerMutationProbabilityOfPolyclonality = {
      ComputePerMutationProbabilityOfPolyclonality(
          pairwiseGenealogy, logMutationPlacementProbabilities, nMutations,
          nCells)};

  std::vector<double> distanceVectorDouble(distanceVector.begin(),
                                           distanceVector.end());
  std::vector<double> splittingStatsDouble(splittingStats.begin(),
                                           splittingStats.end());
...
/** Takes the posterior sampling of trees and computes bayesian genotype
 * profiles for single cells.
 *
 *
 ...
 * stands for being a white blood cell.
 *
 * @output An array that indicates the posterior probability for a mutation
 * (column) to be there in a cell (row).
 *
 */
// [[Rcpp::export]]
std::vector<std::vector<double>> getProbabilityOfBeingMutated(
    std::vector<Rcpp::DataFrame> treeData, int nCells, int nMutations,
    int nClusters, const std::vector<int> &alleleCount,
    const std::vector<int> &ClusterID,
    const std::vector<std::vector<int>> &mutatedReadCounts,
    const std::vector<std::vector<int>> &totalReadCounts,
  ...
  std::vector<std::vector<double>> logCellGenotypeMatrix(
      nCells, std::vector<double>(nMutations, 0.0));
  ...
  for (int treeIndex = 0; treeIndex < nTrees; treeIndex++) {
    ...
    double seqErrRate =
        Rcpp::as<double>(treeData[treeIndex]["SequencingErrorRate"]);

    std::vector<std::vector<bool>> ancestorMatrix = parentVector2ancMatrix(
        treeParentVectorFormat, treeParentVectorFormat.size());
    std::vector<std::vector<double>> logMutationDistribution =
        computeMutationDistribution(nCells, nMutations, nClusters,
    ...
    for (int mutIndex = 0; mutIndex < nMutations; mutIndex++) {
      ...
      for (int nodeIndex = 0; nodeIndex < 2 * nCells - 1; nodeIndex++) {
        mutationDistribution.back().push_back(
            exp(logMutationDistribution[mutIndex][nodeIndex]));
      }
      mutationDistribution.back() =
          normalizeDistribution(mutationDistribution.back());
    }

    for (int cellIndex = 0; cellIndex < nCells; cellIndex++) {
      std::vector<int> lineage = findMostRecentCommonAncestor(
          treeParentVectorFormat, cellIndex, cellIndex)[0];

      std::vector<double> mutationProbabilitiesVector;
      for (int mutIndex = 0; mutIndex < nMutations; mutIndex++) {

        double mutationProbabilityTemp = 0;
        for (int lineageIndex = 0; lineageIndex < lineage.size();
             lineageIndex++) {
          mutationProbabilityTemp +=
              mutationDistribution[mutIndex][lineage[lineageIndex]];
        }

        logCellGenotypeMatrix[cellIndex][mutIndex] =
            log(exp(logCellGenotypeMatrix[cellIndex][mutIndex]) +
                mutationProbabilityTemp);
      }
    }
  }
  ...
  for (int cellIndex = 0; cellIndex < nCells; cellIndex++) {
    for (int mutIndex = 0; mutIndex < nMutations; mutIndex++) {
      logCellGenotypeMatrix[cellIndex][mutIndex] =
          logCellGenotypeMatrix[cellIndex][mutIndex] - log(nTrees);
    }
  }
