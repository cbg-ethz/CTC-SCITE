...
void printGeneFrequencies(int **dataMatrix, int n, int m,
                          vector<string> geneNames);
vector<string> getGeneNamesRecMut(string fileName, int nOrig,
...
void filterLowReadCountSites(vector<vector<int>> &readCounts,
                             vector<string> &mutIds, int minCellCount,
                             int minCov);
...
double defaultMoveProbsBin[] = {
    0.4, 0.6}; // default tree move probs: prune&re-attach / swap leaf labels
...
/**** MCMC  ****/
bool useFixedSeed =
    false; // use a predefined seed for the random number generator
...
// char treeType = 'm';        // the default tree is a mutation tree; other
// option is 't' for (transposed case), where we have a binary leaf-labeled tree
// int maxTreeListSize = -1;  // defines the maximum size of the list of optimal
// trees, default -1 means no restriction bool doubletModel = false; double
// doubletProb = 0.0; bool fixedDoubletProb = false; int recMut = -1; // default
// case no recurrent mutation, set to number of doublet mutation if applicable
// bool recMutAllowed = false;     // no recurrent mutation allowed
// int z = 0;                    //100000000;     // number of positions
// sequenced with no mutation called in any of the samples
...
int main(int argc, char *argv[]) {
  ...
  // std::vector<struct treeTheta> optimalTrees;        // list of optimal
  // tree/beta combinations found by MCMC

  /**  read parameters and data file  **/
  readParameters(argc, argv);

  /* initialize the random number generator, either with a user defined seed, or
   * a random number */
  ...
  vector<bool>
      wbcStatus; // wbcStatus[i] is 1 if leaf i represents a wbc, 0 otherwise
  ...
  // filterLowReadCountSites(readCounts, mutLabels, 2, 3);             // filter
  // out positions that are likely sequencing errors
  ...
  sampleInfo = readStringTableFile(
      sampleNameFile); // the original table of the samples (ids and cell count)
  ...
  // getchar();
  cout << "before MCMC\n";
  runMCMCnew(rep, loops, gamma_, moveProbs, n, m, sampleCount, alleleCount,
             leafClusterId, mutReadCounts, totalReadCounts, dropOut, seqErr,
             tau, label, treeFiles, mutLabels, wbcStatus, prefix, sampleStep,
             burnin);

  //	for(int i = 0; i < 10000; i++){
  //		int*  currTreeParentVec  = getRandomBinaryTree(m);     // sample
  // a random binary tree 		bool** ancMatrix =
  // parentVector2ancMatrix(currTreeParentVec, (2*m)-2);
  //		//cout << getGraphVizBinTree(currTreeParentVec, m, label);
  //		writeToFile(getGraphVizBinTree(currTreeParentVec, m, label),
  //"/Users/jahnka/Desktop/AcetoData/CTC-Exome_vcf_180105/Br23.gv");
  ...
  //		double score = scoreTree(m, n, sampleCount, ancMatrix,
  // alleleCount, leafClusterId, mutReadCounts, totalReadCounts, dropOut,
  // seqErr,
  // tau); 		cout
  //<< score << endl; 		cout << "...\n";
  // free_boolMatrix(ancMatrix); 		delete [] currTreeParentVec;
  //	}
  // getchar();
  ...
  // sampleOutput = runMCMCnew(optimalTrees, errorRates, rep, loops, gamma_,
  // moveProbs, n, m, dataMatrix, scoreType, trueParentVec, sampleStep, sample,
  // chi, priorSd_beta, priorSd_alpha, useTreeList, treeType, doubletModel,
  // doubletProb, recMut, z, alphaMoveRatio, fixedDoubletProb);
  ...
  //	if(treeType=='t'){parentVectorSize = (2*m)-2;}   // transposed case:
  // binary tree, m leafs and m-1 inner nodes, root has no parent
  //
  //	int outputSize = optimalTrees.size();
  //
  //
  //	//vector<string> geneNamesList = getGeneNamesRecMut(geneNameFile, n,
  // recMutAllowed, recMut); 	vector<string> sampleNamesList =
  // getSampleNames(sampleNameFile, m);
  ...
  //		bool** ancMatrix = parentVector2ancMatrix(parentVector,
  // parentVectorSize); 		vector<vector<int> > childLists =
  // getChildListFromParentVector(parentVector, parentVectorSize);
  //
  //		// print newick presentation
  //		stringstream newick;
  //		string outputFile = getFileName2(i, prefix, ".newick",
  // scoreType); 		newick << getNewickCode(childLists,
  // parentVectorSize)
  // <<
  // "\n";
  //		//newick <<  getNewickCodeMutNames(childLists, parentVectorSize,
  // nodeLabels); 		writeToFile(newick.str(), outputFile);
  //		//cout << newick.str() << "\n";
  //
  //		// print GraphViz representation
  //		outputFile = getFileName2(i, prefix, ".gv", scoreType);
  ////		if(errorRateMove != 0.0){
  ////			updateLogScoresAlphaBeta(logScores,
  /// optimalTrees[i].beta, optimalTrees[i].beta); /		}
  //
  //		if(treeType == 'm'){
  //			string output;
  //
  //			//print_intArray(parentVector, parentVectorSize);
  //	//		output = getMutTreeGraphVizString(parentVector,
  // parentVectorSize, geneNamesList, attachSamples, sampleNamesList,
  // dataMatrix, logScores, recMutAllowed, recMut);
  //
  //			//output = getGraphVizFileContentNames(parentVector,
  // parentVectorSize, geneNamesList, attachSamples, ancMatrix, m, logScores,
  // dataMatrix); 			writeToFile(output, outputFile);
  //		}
  //		else{
  //	//		int* bestPlacement =
  // getHighestOptPlacementVector(dataMatrix, n, m, logScores, ancMatrix);
  //
  //	//		vector<string> bestBinTreeLabels =
  // getBinTreeNodeLabels((2*m)-1, bestPlacement, n, geneNamesList,
  // sampleNamesList);
  ...
  ////			}
  ////			//string temp =
  /// getGraphVizBinTree(optimalTrees.at(0).tree, (2*m)-1, m,
  /// bestBinTreeLabels); /			string temp =
  /// getBinTreeGraphVizString(optimalTrees.at(i).tree, parentVectorSize,
  /// bestBinTreeLabels, sampleNamesList);
  ////			writeToFile(temp, outputFile);
  ////			//cout <<
  /// getNewickCodeGeneNames(getChildListFromParentVector(parentVector,
  /// parentVectorSize), (2*m)-1, bestBinTreeLabels);
  //		}
...
vector<string> getSampleNames(string fileName, int count) {
  ...
  if (!in) {
    cout << "Can't find sample names file " << fileName
         << ", "; // in case no labels are given, use sample ids instead
...
vector<string> getGeneNamesRecMut(string fileName, int nOrig,
  ...
  if (!in) {
    cout << "Cannot open gene names file " << fileName
         << ", "; // in case no labels are given, use node ids instead
    cout << "using ids instead.\n";
    vector<string> empty;
...
int getMinDist(int *trueVector, std::vector<bool **> optimalTrees, int n) {
  ...
  for (int i = 0; i < optimalTrees.size(); i++) {
    int dist = getSimpleDistance(
        trueVector, ancMatrixToParVector(optimalTrees.at(i), n), n);
...
int readParameters(int argc, char *argv[]) {
  for (int i = 1; i < argc; ++i) {
    ...
    if (strcmp(argv[i], "-i") == 0) { // name of data file
      if (i + 1 < argc) {
        fileName = argv[++i];
               ...
               0) { // name of file describing the samples
      if (i + 1 < argc) {
        sampleNameFile = argv[++i];
      }
    } else if (strcmp(argv[i], "-o") ==
               0) { // (optional) output file if not generic name created from
                    // input file name
      if (i + 1 < argc) {
        outFile = argv[++i];
      }
    ...
    } else if (strcmp(argv[i], "-g") == 0) { // gamma, default 1, only adjust
                                             // when NOT sampling from posterior
      if (i + 1 < argc) {
        gamma_ = atof(argv[++i]);
      }
    } else if (strcmp(argv[i], "-e") == 0) { // probability of MCMC to pick move
                                             // for proposing new error rate
      if (i + 1 < argc) {
        errorRateMove = atof(argv[++i]);
      }
    } else if (strcmp(argv[i], "-p") ==
               0) { // set the step size for sampling from posterior and invoke
                    // sampling
      if (i + 1 < argc) {
    ...
    } else if (strcmp(argv[i], "-move_probs") ==
               0) { // sets the probabilities of different tree moves in MCMC
      ...
      //					//std::cerr << "recurrent mut "
      //<< std::endl; 					if (i + 1 < argc &&
      // strncmp(argv[i+1], "-", 1)!=0) {
      // recMutAllowed = true;
      //						//std::cerr <<
      //"recurrent mut " << std::endl;
      // recMut = atoi(argv[++i]);
      //					}
    } else if (strcmp(argv[i], "-seed") == 0) {
      useFixedSeed = true;
      if (i + 1 < argc) {
        fixedSeed = atoi(argv[++i]);
      }
...
vector<double> setMoveProbs() {
  ...
  if (treeMoves.size() == 0) { // use default probabilities
    ...
    if (sum != 1.0) {
      cerr << "move probabilities do not sum to 1.0, recalculating "
...
vector<string> readMutIds(string fileName) {
  ...
  while (not_done) {
    ...
    stringstream id;
    id << token_1 << "_" << token_2 << "_" << token_3 << token_4;
    v.push_back(id.str());
    if (!getline(in, line)) {
      not_done = false;
    }
...
int **getAlpha(int clusterCount, int *parentVec, int parentVectorSize,
               vector<int> &leafClusterId) {

  int **alpha = init_intMatrix(clusterCount, parentVectorSize + 1,
                               0); // init counts with zero
