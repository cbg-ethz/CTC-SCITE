...
#define UNIFORM_BETA_DISTR_seqErr                                              \
  false // if true, use uniform distribution for the prior of seqErr
#define UNIFORM_BETA_DISTR_dropout                                             \
  false // if true, use uniform distribution for the prior of the dropout rate
...
/* a new value for the sequencing error is sampled from a normal distribution
 * around the current error rate */
...
double proposeNewLogTau(double currLogTau, int currTauCount,
                        double currLogTausSum, double currLogTausSquaredSum) {
  double first = currLogTausSquaredSum / currTauCount;
  double second =
      ((currLogTausSum * currLogTausSum) / (currTauCount * currTauCount)) /
      currTauCount;
  double stdev = sqrt(first - second);
  if (first - second < 0) {
    cout << "sqrt(" << first << "," << second << ")" << endl;
    // cout << "stdev = sqrt(" << (currLogTausSquaredSum/currTauCount) -
    // ((currLogTausSum*currLogTausSum)/(currTauCount*currTauCount)) << ")" <<
    // endl;
    cout << "stdev: " << stdev << "    logTauSum: " << currLogTausSum
         << "    squared: " << currLogTausSquaredSum
         << "   count: " << currTauCount << endl;
    getchar();
  }

  cout << "stdev = sqrt("
       << (currLogTausSquaredSum / currTauCount) -
              ((currLogTausSum * currLogTausSum) /
               (currTauCount * currTauCount))
       << ")" << endl;
  cout << "stdev: " << stdev << "    logTauSum: " << currLogTausSum
       << "    squared: " << currLogTausSquaredSum
       << "   count: " << currTauCount << endl;
  ...
  //			cout << logSquareSum.at(i) << "\t" << logSum.at(i) <<
  // endl;
  //	}
  // getchar();
  return newLogTau;
...
std::string runMCMCnew(int noOfReps, int noOfLoops, double gamma_,
                       vector<double> moveProbs, int n, int m, int sampleCount,
                       vector<int> &alleleCount, vector<int> &leafClusterId,
                       vector<vector<int>> &mutReadCounts,
                       vector<vector<int>> &totalReadCounts, double dropoutRate,
                       double seqErr, double tau, vector<string> &label,
                       vector<string> &treeFiles, vector<string> mutLabels,
                       vector<bool> &wbcStatus, string prefix, int step,
                       int burnIn) {

  // Beta_Distr seqErrPrior(seqErr, 0.1, UNIFORM_BETA_DISTR_seqErr);          //
  // prior distribution sequencing error Beta_Distr dropOutPrior(dropoutRate,
  // 0.1, UNIFORM_BETA_DISTR_dropout);   // prior distribution allelic dropout
  ...
  // int** postSamplingMatrix = init_intMatrix(n, m, 0);   // counts for each
  // (mutation,sample) pair how often the
  ...
  for (int r = 0; r < noOfReps;
    ...
    currTreeLogScore =
        scoreTree(m, n, sampleCount, currTreeAncMatrix, alleleCount,
                  leafClusterId, mutReadCounts, totalReadCounts, dropoutRate,
    ...
    for (int it = 0; it < noOfLoops; it++) { // run the iterations of the MCMC
      ...
      // cout << "it=" << it << "   burnin=" << burnIn << "   step=" << step <<
      // endl;
      if (doSampling && it >= burnIn &&
          it % step == 0) { // set to true if need to sample in this step
        sampleStep = true;
      } else {
        sampleStep = false;
      }
      ...
      bool moveChangesParams = changeSeqErr(
          moveProbs[0]); // true if this move changes theta, not the tree

      if (moveChangesParams) { // new theta is proposed, log scores change, tree
        ...
        if (paramType == 's') {
          propSeqErrRate =
              proposeNewSeqErr(currSeqErrRate, 0.1 * currSeqErrRate);
        } else if (paramType == 'd') {
          propDropOutRate =
              proposeNewSeqErr(currDropOutRate, 0.1 * currDropOutRate);
        }
        // else{
        //	propLogTau = proposeNewLogTau(currLogTau, currTauCount,
        // currLogTausSum, currLogTausSquaredSum); cout << "proposed: " <<
        // exp(propLogTau) << endl;
        //}
        ...
        propTreeLogScore = scoreTree(
            m, n, sampleCount, currTreeAncMatrix, alleleCount, leafClusterId,
            mutReadCounts, totalReadCounts, propDropOutRate, propSeqErrRate,
            exp(propLogTau), bestPlacementPoint, wbcStatus);

        // cout << propTreeLogScore << "\t" << currTreeLogScore << "\t" <<
        // bestTreeLogScore << endl;

        isAcceptedMove =
            sample_0_1() < exp((propTreeLogScore - currTreeLogScore) * gamma_);
        ...
        if (isAcceptedMove) {                  // the proposed move is accepted
          ...
          // logSquareSum.push_back(currLogTausSquaredSum);
          // logSum.push_back(currLogTau);
          // currScore = currTreeLogScore;                          // combined
          // score of current tree and current beta cout << "new tau: " <<
          // exp(currLogTau) << endl; cout << "new param" << endl;
        ...
        bool **propTreeAncMatrix =
            parentVector2ancMatrix(propTreeParVec, parentVectorSize);
        propTreeLogScore = scoreTree(
            m, n, sampleCount, propTreeAncMatrix, alleleCount, leafClusterId,
            mutReadCounts, totalReadCounts, currDropOutRate, currSeqErrRate,
            tau, bestPlacementPoint, wbcStatus);
        //        		cout << propTreeLogScore << "\t" <<
        //        currTreeLogScore << bestTreeLogScore << endl;

        isAcceptedMove =
            sample_0_1() <
            nbhcorrection * exp((propTreeLogScore - currTreeLogScore) * gamma_);
      ...
      if (currTreeLogScore > bestScore) {
        // optStatesAfterBurnIn = 0;                    // new opt state found,
        // discard old count
        bestTreeLogScore = currTreeLogScore;
        delete[] bestTreeParentVec;
        bestTreeParentVec =
            deepCopy_intArray(currTreeParentVec, parentVectorSize);
      ...
      if (isAcceptedMove) { // update top tree list if necessary
        ...
        if (insert) {
          string gv = getFancyGraphVizBinTree(
              bestTreeParentVec, m, label, bestTreeLogScore, bestDropOutRate,
              bestSeqErrRate, leafClusterId, bestPlacementPoint, mutLabels);
        ...
        for (it = topTreeList.begin(); it != topTreeList.end();
             ++it) { // write all 10 trees to file
      ...
      if (sampleStep) {
        ...
        // cout << std::setprecision( std::numeric_limits<int>::max() ) <<
        // currSeqErrRate << "\t" << currDropOutRate << "\t" << currLogTau <<
        // "\t" << treeString.str() << endl;
        stringstream parameters;
        parameters << std::setprecision(std::numeric_limits<double>::digits10 +
                                        1)
                   << currTreeLogScore << "\t" << currSeqErrRate << "\t"
                   << currDropOutRate << "\t" << currLogTau;
...
double scoreTree(int m, int n, int sampleCount, bool **ancMatrix,
                 vector<int> &alleleCount, vector<int> &leafClusterId,
                 vector<vector<int>> &mutReadCounts,
                 vector<vector<int>> &totalReadCounts, double dropoutRate,
                 double seqErr, double tau, vector<int> &bestPlacementPoint,
  ...
  for (int mut = 0; mut < n;
    ...
    for (int att = 0; att < (2 * m) - 1; att++) { // for each attachment point
      ...
      if (bestLogAttachmentScore <
        ...
        //				cout << "equally good placement" <<
        // endl; cout << "current best placement: " <<
        // bestPlacementPoint.at(mut)
        // << endl; cout << "   candidate placement: " << att << endl;
        if (att == (2 * m) - 2) {
          bestPlacementPoint.at(mut) = att;
    ...
    double sumScore = 0.0;
    for (int i = 0; i < logAttachmentScores.size();
         i++) { // sum over all allele states, exp is necessary as scores are
                // actually log scores
      sumScore += exp(
          logAttachmentScores.at(i) -
          bestLogAttachmentScore); // subtraction of best score to calculate
                                   // with score differences (smaller values)
    }

    double logMutScore =
        log(sumScore) +
        bestLogAttachmentScore; // transform back to log scores and change from
                                // score differences to actual scores
...
vector<vector<double>> getPlacementScoreTable(
    int m, int n, int sampleCount, bool **ancMatrix, vector<int> &alleleCount,
    vector<int> &leafClusterId, vector<vector<int>> &mutReadCounts,
    vector<vector<int>> &totalReadCounts, double dropoutRate, double seqErr,
    double tau, vector<int> &bestPlacementPoint, vector<bool> &wbcStatus) {
