...
int sampleRandomMove(
    std::vector<double> prob) { // picks randomly one of the tree moves based on
                                // the move probabilities

  double percent = (rand() % 100) + 1; // between 1 and 100
  double probSum = prob[1];
  for (int i = 1; i < prob.size() - 1;
       i++) { // start at index 1; the probability at prob[0] is for changing
              // the error rate (which is treated separately)
...
int pickRandomNumber(int n) { return (rand() % n); }
...
int *getRandomBinaryTree(int m) {
  int parentCount =
      (2 * m) -
      2; // the m leafs have a parent and so have m-2 of the m-1 inner nodes
  int *leafsAndInnerNodesParents = init_intArray(parentCount, -1);

  std::vector<int> queue;
  for (int i = 0; i < m; i++) {
    queue.push_back(i);
  } // add the m leafs to the queue
