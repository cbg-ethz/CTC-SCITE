...
/* proposes a new binary tree by a single move from the current binary tree
 * based on the move probabilities */
...
int *proposeNextBinTree(std::vector<double> moveProbs, int m,
  ...
  int movetype = sampleRandomMove(
      moveProbs); // pick the move type according to move probabilities
  ...
  vector<vector<int>> childLists =
      getChildListFromParentVector(currTreeParVec, parVecLength);
  ...
  if (movetype == 1) { /* type 1: prune and re-attach */
    ...
    int sib = getSibling(
        v, currTreeParVec,
        childLists); // get the sibling of node v and attach it to the
    propTreeParVec[sib] = currTreeParVec[p]; // grandparent of v, as the parent
                                             // of v is moved along with v

    std::vector<int> possibleSiblings = getNonDescendants(
        currTreeAncMatrix, p,
        parVecLength); // get list of possible new siblings of v

    if (possibleSiblings.size() == 0) {
      cerr << "Error: No new sibling found for node " << v
           << " for move type 1 in binary tree.\n"; // Should never occur.
                                                    // Something wrong with the
                                                    // tree.
      // printGraphVizFile(currTreeParVec, parVecLength);
    }

    int newSibling = possibleSiblings[pickRandomNumber(
        possibleSiblings.size())]; // pick a new sibling from remaining tree
                                   // (root can not be a sibling)
    propTreeParVec[newSibling] =
        p; // make the new sibling a child of v's parent
    propTreeParVec[p] =
        currTreeParVec[newSibling]; // make the parent of v the child of the new
                                    // sibling's former parent
  } else {                          /* type 2: swap two node labels  */
    // cout << "move type is swap node labels in binary tree\n";
    // print_intArray(propTreeParVec, parVecLength);
    int v = rand() %
            m; // get random leaf to swap (only the first m nodes are leafs)
...
int pickNodeToMove(int *currTreeParentVec, int parentVectorLength) {
  ...
  while (!validNode) {
    int v = pickRandomNumber(
        parentVectorLength); // pick a node for the prune and re-attach step;
    if (currTreeParentVec[v] !=
        rootId) { // it has to be a node whose parent is not the root, as node
                  // and parent are moved together
      return v;
