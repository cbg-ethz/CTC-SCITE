...
void updateTreeList(vector<struct treeTheta> &bestTrees, int *currTreeParentVec,
                    int n, double currScore, double bestScore, double beta,
                    double alpha) {

  if (currScore > bestScore) {
    // cout << "tree list of size " << bestTrees.size() << " emptied\n";
    resetTreeList(
        bestTrees, currTreeParentVec, n, beta,
        alpha); // empty the list of best trees and insert current tree

  } else if (currScore == bestScore) {
    if (!isDuplicateTreeFast(bestTrees, currTreeParentVec,
                             n)) { // if the same tree was not previously found
      treeTheta newElem =
          createNewTreeListElement(currTreeParentVec, n, beta, alpha);
...
void resetTreeList(vector<struct treeTheta> &bestTrees, int *newBestTree, int n,
...
struct treeTheta createNewTreeListElement(int *tree, int n, double beta,
                                          double alpha) {
  treeTheta newElem;
...
bool isDuplicateTreeFast(std::vector<struct treeTheta> &optimalTrees,
                         int *newTree, int n) {
