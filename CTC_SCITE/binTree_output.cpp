...
/* Score contribution by a specific mutation when placed at the root, that means
 * all samples should have it */
...
/* computes the best placement of a mutation, the highest one if multiple
 * co-optimal placements exist*/
int getHighestOptPlacement(int **obsMutProfiles, int mut, int m,
                           double **logScores, bool **ancMatrix) {

  int nodeCount = (2 * m) - 1;
  int bestPlacement = (2 * m) - 2; // root
  double bestPlacementScore =
      binTreeRootScore(obsMutProfiles, mut, m, logScores);
  // cout << bestPlacementScore << " (root)\n";
  // print_boolMatrix(bool** array, int n, int m);
  for (int p = 0; p < nodeCount - 1;
       p++) { // try all possible placements (nodes in the mutation tree)

    double score = 0.0; // score for placing mutation at a specific node
    for (int sample = 0; sample < m; sample++) {
      // cout << p << " " << sample << "\n";
      if (ancMatrix[p][sample] == 1) {
        score += logScores[obsMutProfiles[sample][mut]]
                          [1]; // sample should have the mutation
      } else {
        score += logScores[obsMutProfiles[sample][mut]]
                          [0]; // sample should not have the mutation
      }
    }
    if (score > bestPlacementScore) {
      ...
      // cout << bestPlacementScore << " (non-root)\n";
               ...
               ancMatrix[p][bestPlacement] == true) {
      bestPlacement = p;
    }
  }
...
int *getHighestOptPlacementVector(int **obsMutProfiles, int n, int m,
  ...
  for (int mut = 0; mut < n; mut++) {
    bestPlacements[mut] =
        getHighestOptPlacement(obsMutProfiles, mut, m, logScores, ancMatrix);
  }
...
/* returns the lca of a node that has a non-empty label, the root is assumed to
 * always have a label */
int getLcaWithLabel(int node, int *parent, vector<string> label,
                    int nodeCount) {
...
string getBinTreeGraphVizString(int *parentVector, int parentVectorSize,
  ...
  str << "node [color=" << nodeColors.at(0)
      << ", style=filled, fontcolor=black, shape=circle];  \n"; // sample nodes
...
void updateInnerNodeLabels(vector<string> &label, int n, int m,
...
string getGraphVizBinTree(int *parents, int m, vector<string> &label,
  ...
  content << "node [color=black, shape=box, fontcolor=black, fontsize=18, "
             "fontname=Verdana],fontcolor=black;\n";
  content << "scoreBox [label= \"Tree score = " << score
          << "\n Dropout rate = " << dropoutRate
          << "\n Sequencing error rate = " << seqErrorRate << "\"];\n";
  // updateInnerNodeLabels(label, bestPlacementPoint.size(), m,
  // bestPlacementPoint);
  updateInnerNodeLabelsMutIds(label, bestPlacementPoint.size(), m,
                              bestPlacementPoint, mutLabels);
  ...
  content
      << "node [color=" << nodeColors.at(0)
      << ", style=filled, fontcolor=black, shape=oval];  \n"; // sample nodes
  for (int i = 0; i < m; i++) {
  ...
  for (int i = leafCount; i < (2 * leafCount) - 2; i++) {
    ...
    for (int leaf = 0; leaf < leafCount; leaf++) {
      ...
      if (ancMatrix[i][leaf] == 1) {
        ...
        string token = s.substr(s.find(l_delim) + 8,
                                s.find(r_delim) - (s.find(l_delim) + 9));
...
string getFancyGraphVizBinTree(int *parents, int m, vector<string> &label,
  ...
  content << "node [color=gray, shape=box, fontcolor=black, fontsize=20, "
             "fontname=Helvetica];\n";
  content << "scoreBox [label= \"Tree score = " << score
          << "\n Dropout rate = " << dropoutRate
          << "\n Sequencing error rate = " << seqErrorRate << "\"];\n";
  ...
  // content << "node [color=" << nodeColors.at(0) << ", style=filled,
  // fontcolor=black, shape=oval];  \n";   // sample nodes
  content << "node "
             "[fontname=helvetica,fontcolor=black,shape=\"box\",penwidth=10,"
             "style=\"rounded,filled,bold\",imagepos=\"bc\",imagescale=true, "
             "labelloc=b]; \n";
  for (int i = 0; i < m; i++) {
    // cout << "getting color for " << i << "\n";
    string nodeColor = getNodeColor(i, currColor, clusterId);
    //	cout << nodeColor << endl;
    //	cout << i << " [color=" << nodeColor << ", label=" << label[i] <<
    //"];\n"; 	content << i << " [color=" << nodeColor << ", label=" <<
    // label[i]
    //<< "];\n";
    content << i << label[i] << ";\n";
    // cout << label[i] << endl;
