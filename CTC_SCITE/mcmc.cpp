...
double logBetaPDF(double x, double bpriora, double bpriorb) {
  double logScore = lgamma(bpriora + bpriorb) + (bpriora - 1) * log(x) +
                    (bpriorb - 1) * log(1 - x) - lgamma(bpriora) -
                    lgamma(bpriorb); // f(x,a,b) = gamma(a+b)/(gamma(a)gamma(b))
                                     // * x^(a-1) * (1-x)^(b-1)
  // cout << lgamma(bpriora+bpriorb) << "   " << tgamma(bpriora+bpriorb) << "
  // #1\n"; cout << (bpriora-1)*log(x) << "  #2\n"; cout << (bpriorb-1)*log(1-x)
  // << "  #3\n"; cout << log(tgamma(bpriora)) << "  #4\n"; cout <<
  // log(tgamma(bpriorb)) << "  #5\n"; cout << logScore << "\n";
  return logScore;
}

/* a new value for the error probability beta is sampled from a normal
 * distribution around the current beta */
...
/* samples a new value for beta from a normal distribution around the current
 * value */
 ...
 * distribution */
string sampleFromPosterior(double currTreeLogScore, int n,
                           int *currTreeParentVec, double betaProb,
...
/* updates the minimum distance between any of the optimal trees and the true
 * tree (if available) */
int updateMinDistToTrueTree(int *trueParentVec, int *currTreeParentVec,
                            int length, int minDistToTrueTree, int currScore,
                            int bestScore) {

  int currDistToTrueTree =
      getSimpleDistance(trueParentVec, currTreeParentVec, length);
  ...
  if (currScore == bestScore &&
      currDistToTrueTree <
          minDistToTrueTree) { // the current tree is closest to the true tree
                               // among the current optimal trees
    return currDistToTrueTree;
  }
