...
double logBetaPDF(double x, double bpriora, double bpriorb) {
	double logScore = lgamma(bpriora + bpriorb) + (bpriora - 1) * log(x)
			+ (bpriorb - 1) * log(1 - x) - lgamma(bpriora) - lgamma(bpriorb); // f(x,a,b) = gamma(a+b)/(gamma(a)gamma(b)) * x^(a-1) * (1-x)^(b-1)
...
string sampleFromPosterior(double currTreeLogScore, int n,
		int* currTreeParentVec, double betaProb, double currBeta,
		double currScore) {
...
int updateMinDistToTrueTree(int* trueParentVec, int* currTreeParentVec,
		int length, int minDistToTrueTree, int currScore, int bestScore) {

	int currDistToTrueTree = getSimpleDistance(trueParentVec, currTreeParentVec,
			length);
