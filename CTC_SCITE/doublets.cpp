...
double doubletRatePrecision = 1e-6;    // defines to what precision of doublet rate should be optimized
...
std::string runMCMCnew(vector<struct treeTheta>& bestTrees, double* errorRates, int noOfReps, int noOfLoops, double gamma_, vector<double> moveProbs, int n, int m, int** dataMatrix, char scoreType, int* trueParentVec, int step, bool sample, double chi, double priorSd_beta, double priorSd_alpha, bool useTreeList, char treeType, bool doubletModel, double doubletRate, int recMut, int z, double alphaMoveRatio, bool fixedDoubletProb){
	...
	for(int r=0; r<noOfReps; r++){   // repeat the MCMC, start over with random tree each time, only best score and list of best trees is kept between repetitions
		...
		double currThetaLogScore = currBetaLogScore + currAlphaLogScore + m*z*log(1-currAlpha);
		...
		for(int it=0; it<noOfLoops; it++){                                     // run the iterations of the MCMC
        	...
        	if(moveChangesTheta){                     // new theta is proposed, log scores change, tree is copy of current tree
        		...
        		double propThetaLogScore =  m*z*log(1-propAlpha) + propBetaLogScore + propAlphaLogScore;
