...
/* Note: This works only if the recursion is started with the root node which is
 * n+1 */
...
/* This converts a tree given as child list vectors to the Newick format using
 * the mutations placed at a node as the label */
string getNewickCodeMutNames(vector<vector<int>> list, int root,
  ...
  cout << root << endl;
  if (nodeLabels.at(root).length() > 0) {
    newick << nodeLabels.at(root);
  } else {
    newick << root + 1;
  }
  return newick.str();
...
int *prueferCode2parentVector(int *code, int codeLength) {
  ...
  int *lastOcc =
      getLastOcc(code, codeLength); // node id -> index of last occ in code, -1
                                    // if no occurrence or if id=root
  bool *queue = getInitialQueue(
      code, codeLength); // queue[node]=true if all children have been attached
                         // to this node, or if it is leaf
  int queueCutter = -1;  // this is used for a node that has been passed by the
                         // "queue" before all children have been attached
  int next = getNextInQueue(queue, 0, codeLength + 1);

  for (int i = 0; i < codeLength;
       i++) { // add new edge to tree from smallest node with all children
              // attached to its parent
    if (queueCutter >= 0) {
      parent[queueCutter] = code[i]; // this node is queueCutter if the queue
                                     // has already passed this node
      // cout << queueCutter << " -> " << code[i] << "\n";
      queueCutter = -1;
    } else {
      parent[next] =
          code[i]; // use the next smallest node in the queue, otherwise
      // cout << next << " -> " << code[i] << "\n";
      next = getNextInQueue(queue, next + 1,
                            codeLength +
                                1); // find next smallest element in the queue
    }

    if (lastOcc[code[i]] ==
        i) { // an element is added to the queue, or we have a new queueCutter
...
/* creates the parent vector for a star tree with node n as center and 0,...,n-1
 * as leafs */
