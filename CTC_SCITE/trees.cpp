...
/* This converts a tree given as child list vectors to the Newick format using the mutations placed at a node as the label */
string getNewickCodeMutNames(vector<vector<int> > list, int root, vector<string> nodeLabels){
	stringstream newick;
	vector<int> rootChilds = list.at(root);
	if(!rootChilds.empty()){
		newick << "(";
