...
double findBestTreeByExhaustiveSearch(int n, int m,
                                      std::vector<int *> &bestTrees,
                                      double **logScores, int **dataMatrix,
                                      char scoreType) {
  ...
  for (int treeno = 1; treeno <= noOfTrees; treeno++) {
    ...
    double newTreeScore =
        scoreTree(n, m, logScores, dataMatrix, scoreType, parent, bestScore);

    if (newTreeScore > bestScore) { // case 1: the current tree has better score
                                    // than previous best tree
      bestScore = newTreeScore;     // update best score
      printf("new best score: %e\n", newTreeScore);
      emptyTreeList(bestTrees,
                    n); // empty the list of best trees (ancestor matrices)
      bestTrees.push_back(parent); // current tree is now only best tree
    } else if (newTreeScore == bestScore) { // case 2: the current tree is
                                            // equally good as best tree so far
      bestTrees.push_back(parent); // add it to list (duplicates not possible
                                   // due to redundancy free enumeration)
...
int *treeNumber2prueferCode(int treeno, int size) {
  int codeLength =
      size - 1; // the length of the pruefer code is the treesize-2, since we
                // add the root as node, we have an offset of 1
  int base =
      size +
      1; // the tree has n+1 nodes, this is the base for constructing the code
  int *code = new int[codeLength];
  int currentno = treeno - 1; // trees are numbered starting from 1
  for (int i = codeLength - 1; i >= 0;
       i--) { // transforms the treeno (base 10) to the pruefer code (base n+1)
...
/*bool** codeToAncMatrix(int treeno, int size)
{
    ...
    bool** ancMatrix = init_boolMatrix(size, size, 0);
        ...
        int novelLeaf[size+1];  //   code index of last occ of leaf x -> x, -1
if not a last occ bool isLeaf[size+1];
...
novelLeaf[lastOcc[i]]+1);
        ...
        int curr = 1;                 // currently the smallest old leaf (apart
from the novel leaf) int novel = size+10;         // node that just became a
leaf after previous step for(int i=0; i<= size; i++){ if(isLeaf[i] == true){
                        curr = i;             // the smallest leaf in the
beginning break;
                }
        }
        ...
        for(int i=0; i<codeLength; i++){
                if(novel < curr){        // new child node is novel leaf
                        ...
                        if(novel<size && code[i]<size){
                                ancMatrix[code[i]][novel] = true; // set new
edge in matrix
                        }
                        //printf("a: %d -> %d\n", novel+1, code[i]+1);
                        for(int j=0; j<size; j++){                  // set new
connections if(j<size && code[i]<size){ ancMatrix[code[i]][j] =
ancMatrix[novel][j] || ancMatrix[code[i]][j];
                                }
...
code[i]+1);
                        ...
                        novel = size+10;                           // novel leaf
is no longer novel if(novelLeaf[i] >= 0){ isLeaf[novelLeaf[i]] = true;     //
update leaf list if there is a new leaf novel = novelLeaf[i];            //
update novel
                        ...
                        if(curr<size && code[i]<size){
                                if(curr<size && code[i]<size){
                                        ancMatrix[code[i]][curr] = true; // set
edge in matrix
                                        //printf("%d -> %d set\n", curr+1,
code[i]+1);
                                }
                        ...
                        for(int j=0; j<size; j++){                  // set new
connections if(j<size && code[i]<size){ ancMatrix[code[i]][j] =
ancMatrix[curr][j] || ancMatrix[code[i]][j];
                                }
                                if(ancMatrix[code[i]][j]==true){
                                        //printf("b: [%d,%d] set to 1 left:
%d,%d      right: %d,%d\n", j+1, code[i]+1, j, novel+1, j+1, code[i]+1);
                                }
                        }
                        novel = size+10;
                        if(novelLeaf[i] >= 0){
                                isLeaf[novelLeaf[i]] = true;     // there is a
new leaf novel = novelLeaf[i];            // update novel
                        }

                        do {                                   // update curr by
finding curr++;                        // the next smallest leaf }
