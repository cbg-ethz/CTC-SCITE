...
double epsilon =
    0.000000000001; // how much worse than the current best a score can be to
                    // still use the accurate score computation

/****     Tree scoring     ****/

/* Computes the score of a new candidate tree. First a fast approximate score is
 * computed, then if the new score is better,   */
/* or slightly worse than the best score so far, a more accurate but more costly
 * score computation is done.                    */
double scoreTree(int n, int m, double **logScores, int **dataMatrix, char type,
                 int *parentVector, double bestTreeLogScore) {

  double approx = scoreTreeFast(n, m, logScores, dataMatrix, type,
                                parentVector); // approximate score

  if (approx >
      bestTreeLogScore - epsilon) { // approximate score is close to or better
    return scoreTreeAccurate(
        n, m, logScores, dataMatrix, type,
        parentVector); // than the current best score, use accurate
  }                    // score computation
 ...
 * can occur  */
double scoreTreeFast(int n, int m, double **logScores, int **dataMatrix,
                     char type, int *parentVector) {

  double result = -DBL_MAX;
  int *bft = getBreadthFirstTraversal(
      parentVector, n); // get breadth first traversal for simple scoring
                        // by updating the parent score
  if (type == 'm') {
    result = maxScoreTreeFast(n, m, logScores, dataMatrix, parentVector,
                              bft); // score by best attachment point per sample
  } else if (type == 's') {
    result =
        sumScoreTreeFast(n, m, logScores, dataMatrix, parentVector,
                         bft); // score by summing over all attachment points
  }
 ...
 * sample */
double maxScoreTreeFast(int n, int m, double **logScores, int **dataMatrix,
                        int *parent, int *bft) {
  ...
  for (int sample = 0; sample < m; sample++) { // for all samples get
    double *scores = getAttachmentScoresFast(
        parent, n, logScores, dataMatrix[sample], bft); // all attachment scores
...
double sumScoreTreeFast(int n, int m, double **logScores, int **dataMatrix,
                        int *parent, int *bft) {
  ...
  // cout << "sum score: " << sumTreeScore << "\n";
  return sumTreeScore;
}

/* computes the attachment scores of a sample to all nodes in the tree (except
 * root) */
double *getAttachmentScoresFast(int *parent, int n, double **logScores,
                                int *dataVector, int *bft) {
  ...
  for (int i = 1; i <= n;
    ...
    // cout << "                                                   attachment
    // score to node " << i << ": " << attachmentScore[node] << "\n";
  }
...
double rootAttachementScore(int n, double **logScores, int *dataVector) {
  double score = 0.0;
  for (int gene = 0; gene < n;
       gene++) { // sum over log scores for all other nodes in tree
    score += logScores[dataVector[gene]][0]; // none of them is ancestor of the
...
double scoreTreeAccurate(int n, int m, double **logScores, int **dataMatrix,
                         char type, int *parentVector) {
  ...
  if (type == 'm') {
    result =
        maxScoreTreeAccurate(n, m, logScores, dataMatrix, parentVector, bft);
  } else if (type == 's') {
    result =
        sumScoreTreeAccurate(n, m, logScores, dataMatrix, parentVector, bft);
  }
...
double maxScoreTreeAccurate(int n, int m, double **logScores, int **dataMatrix,
                            int *parent, int *bft) {

  int **treeScoreMatrix = init_intMatrix(4, 2, 0);
  for (int sample = 0; sample < m; sample++) {
    int **bestAttachmentMatrix = getBestAttachmentScoreAccurate(
        init_intMatrix(4, 2, 0), parent, n, logScores, dataMatrix[sample], bft);
...
double sumScoreTreeAccurate(int n, int m, double **logScores, int **dataMatrix,
                            int *parent, int *bft) {

  double sumTreeScore = 0.0;

  for (int sample = 0; sample < m; sample++) {
    sumTreeScore += getSumAttachmentScoreAccurate(parent, n, logScores,
...
int **getBestAttachmentScoreAccurate(int **scoreMatrix, int *parent, int n,
                                     double **logScores, int *dataVector,
                                     int *bft) {

  int ***attachmentScoreMatrix = getAttachmentMatrices(
      parent, n, dataVector,
      bft); // matrix to keep attachment scores for each sample (not summing up
            // components to avoid rounding errors)
  double bestScore = -DBL_MAX;
  int **bestScoreMatrix = NULL;

  for (int i = 0; i < n + 1; i++) { // now get true attachment scores and find
                                    // best score among all attachment points
...
double getSumAttachmentScoreAccurate(int *parent, int n, double **logScores,
                                     int *dataVector, int *bft) {
...
int ***getAttachmentMatrices(int *parent, int n, int *dataVector, int *bft) {
  int ***attachmentScoreMatrix =
      new int **[n + 1]; // matrix to keep attachment scores for each sample
                         // (not summing up components to avoid rounding errors)
  ...
  for (int i = 1; i < n + 1; i++) {
    int node = bft[i];
    attachmentScoreMatrix[node] =
        deepCopy_intMatrix(attachmentScoreMatrix[parent[node]], 4, 2);
...
/* computes a table of the log scores of observing one genotype, given that
 * another genotype */
/* is the true one; for three observed types (+missing observation) and two true
 * states */
double **getLogScores(double FD, double AD1, double AD2, double CC) {
  ...
  if (CC != 0.0) {
    ...
    logScores[2][0] = 0.0; // CC=0 should only occur when there are no 2's
                           // (double mutations) in the matrix
  }
...
void updateLogScores(double **logScores, double newAD) {
  ...
  double newAD1 =
      newAD; // the default case: there are no homozygous mutation observed
  double newAD2 = 0.0;

  if (logScores[2][1] != 0) { // other case: homozygous mutations were called
    newAD1 =
        newAD /
        2; // for simplicity we set both dropout rates to 1/2 of the new value
...
void updateLogScoresAlphaBeta(double **logScores, double newAD, double newFD) {
  ...
  logScores[0][0] = log(1.0 - newFD - CC); // observed 0, true 0
  logScores[1][0] = log(newFD);            // observed 1, true 0
  if (CC != 0.0) {
    logScores[2][0] = log(CC); // observed 2, true 0
  }
