...
double getMaxVectorEntry(vector<double> list) {
  double maxEntry = -DBL_MAX;
  for (int i = 0; i < list.size(); i++) {
    maxEntry = max(maxEntry, list[i]);
  }

  return maxEntry;
}

double getSumOfVectorElementsExpLog(vector<double> list) {
  double sum = 0.0;
  double maxEntry = getMaxVectorEntry(list);
  for (int i = 0; i < list.size(); i++) {
    sum += exp(list[i] - maxEntry); // sum over all doublet scores
                                    // sum += exp(list[i]);
  }
  return log(sum) + maxEntry;
  // return log(sum);
}
  ...
  for (int i = 0; i < n; i++) {
    for (int k = 0; k < n; k++) {
      if (k != i && anc[k][i] == true) { // k is true ancestor of i
        bool cand = true;
        for (int l = 0; l < n; l++) {
          if (l != i && l != k && anc[l][i] == true &&
              anc[k][l] == true) { // k is ancestor of l, and l is ancestor of i
            cand = false; // k is no longer candidate for being parent of i
            break;
          }
        }
        if (cand ==
            true) { // no ancestor of i is descendant of k -> k is parent of i
          parVec[i] = k;
        }
      }
    }
